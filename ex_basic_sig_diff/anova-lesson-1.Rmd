...
* `y`: We set this to the same formula input we used for the `aov()` function; `dependent_var ~ independent_var`. *This argument must come first!*
...
Once again, because of way the null and alternative hypotheses are set up, we are in the odd position of hoping that we fail to provide evidence against the null hypothesis. In other words,
...
Running this test in `R` is straightforward. Simply pass the vector of residuals from `iris.aov` into the command `shapiro.test()`. Try it below, and then answer the follow-up questions
...
1. Identified our independent and dependent variables (`Speices` and `Sepal.Width`, respectively);
...
**Optional note on "*population level*"**<br>
...
question("Between which two groups did Tukey's HSD detect a significant difference in means? Use a significance level (i.e. $P$-value threshold) of 0.05. <br> *Select all that apply*.",
...
For example, Tukey's test estimates the difference between the population mean speal width for `versicolor` *minus* the population mean sepal width for `setosa` falls between $-0.819$ inches and $-0.497$ inches.
...
question_rank(
  "According to the confidence intervals from Tukey's Test, sort the iris sepecies by increasing mean sepal width at the population level.<br></br> (*The species with the smallest mean sepal width should be at the **top**, with the means increasing as you move down.*)",
