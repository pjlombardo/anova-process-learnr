...
* `formula`: the same formula input we used for the `aov()` function; `dependent_var ~ independent_var`.
...
```
```{r levene-hint-2}
leveneTest(formula = Sepal.Width ~ ...,
          center = "mean",
          data = iris)
```
```{r levene-solution}
leveneTest(formula = Sepal.Width ~ Species,
          center = "mean",
          data = iris)
```
